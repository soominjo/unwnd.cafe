import type { SalesSummary } from '../types'
import { EmptyState, Skeleton } from './ui'

interface TopCustomersViewProps {
  summary: SalesSummary | null
  loading: boolean
}

/** Most frequent named customers for the period, ranked by order count — the "regulars" list. */
export default function TopCustomersView({ summary, loading }: TopCustomersViewProps) {
  if (loading) return <Skeleton rows={5} height="h-12" />

  const customers = summary?.topCustomers ?? []
  if (customers.length === 0) {
    return <EmptyState title="No named orders in this period." hint="Customers who leave a name at checkout will rank here." />
  }
  const maxOrders = Math.max(1, ...customers.map((c) => c.orderCount))

  return (
    <section className="overflow-hidden rounded-lg border border-border bg-white/80">
      <header className="flex items-baseline justify-between border-b border-border/70 bg-foreground/3 px-4 py-3">
        <h2 className="text-[11px] font-bold uppercase tracking-[0.16em] text-foreground/60">Top customers</h2>
        <span className="text-[10px] uppercase tracking-[0.16em] text-foreground/40">by orders placed</span>
      </header>
      <ol className="divide-y divide-border/60">
        {customers.map((customer, i) => (
          <li
            key={customer.name}
            className="grid grid-cols-[2rem_1fr_auto_6rem] items-center gap-x-4 px-4 py-3 sm:grid-cols-[2rem_1fr_9rem_6rem]"
          >
            <span className="font-display text-sm font-bold tabular-nums text-foreground/30">{String(i + 1).padStart(2, '0')}</span>
            <span className="truncate text-sm font-medium text-foreground">{customer.name}</span>
            <div className="flex items-center gap-2">
              <div className="hidden h-1.5 w-24 overflow-hidden rounded-full bg-foreground/10 sm:block" aria-hidden="true">
                <div className="h-full rounded-full bg-foreground/60" style={{ width: `${(customer.orderCount / maxOrders) * 100}%` }} />
              </div>
              <span className="w-8 text-right text-xs tabular-nums text-foreground/60">×{customer.orderCount}</span>
            </div>
            <span className="text-right text-sm font-semibold tabular-nums text-foreground">₱{customer.totalSpent.toLocaleString()}</span>
          </li>
        ))}
      </ol>
    </section>
  )
}
