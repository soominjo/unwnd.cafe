...
- **Lifted v1 constraints.** The v1 lines "no change to the API route" / "No changes under `app/api/`" and Review Focus 4's "at most one discount row per `lineId`" no longer hold: the route gained the unique-key helper and a larger cap, and a line may carry two rows. Whole-order toggles are dropped when the last line is removed with the minus button, the same as when Clear is tapped.

## Revision 3 (2026-09-23, owner feedback on the panel)

- **Per-item modal replaces the inline rows.** The order panel's Add-ons, Customize and Discount rows crowded the panel and, when Customize and Discount were open for the same line, React saw two siblings with the same `key` (the line id) and duplicated the Customize row. Each line's 📝 / + / % button now opens one standard-size modal (`app/pos/ItemActionModal.tsx`, `max-w-md`, above the mobile drawer) on that section: `CustomizeSection`, `AddonsSection`, `DiscountSection`. A preset or add-on tap applies and closes; discount chips keep the modal open so both kinds can be stacked.
- **Lean header, big price.** The modal shows the item name, then a large `₱160 × 4` with the hot/ice tag, then attached add-ons (they are part of the discount base). Discount badges and the line total are not repeated there, and the sections carry no explanation lines. Each discount row shows its computation, e.g. `PWD Drink −20% ×1 · 20% of ₱165  −₱33`, via `lineDiscountBase`. The strip below shows Subtotal, Discount and Total, then Done.
- **POSClient state.** `customizeLineId` / `addonsLineId` / `discountLineId` became `itemModal: { lineId, mode } | null`; `addAddon` takes its parent line explicitly (the modal's item, or the selected line from the product grid's add-on tiles). Removing the modal's line, Clear, checkout and a back-cache restore all close it.

## Revision 4 (2026-09-23, owner feedback on the modal)

- **Two buttons per line, not three.** The 📝 Customize button is gone; "+" now opens one "Add-ons & Customize" modal holding the add-on grid and the preset chips plus the "Other" field. It fills sky-blue when the line has add-ons *or* a note. The order panel's line rows stop compressing a long item name into three lines.
- **Each mode shows only its own concern.** The Add-ons & Customize modal lists the attached add-ons and ends with "Item total" (line price × qty plus its add-ons) — no discount figures. The Discount modal shows only the item, its price line, the four chips, the computation rows and the order's Subtotal / Discount / Total — no add-ons or note. Because the add-ons are not listed there, a computation row whose base includes them says `· 20% of ₱220 incl. add-ons`.
- **Nothing closes on a tap.** Add-ons, presets and discount chips all apply and leave the modal open so they can be stacked; only Done, ✕, the backdrop or Escape close it.
- **Short badges in the panel.** Line rows read `PWD/S −20% applied` and `GR −10% applied` via `discountBadge(..., 'short')`. The customer-facing review screen keeps the full `PWD/Senior` and `Google Review` names, where there is room and the customer is the reader.

## Revision 5 (2026-09-23, owner feedback on the two modal modes)

- **Chosen add-ons read like chosen presets.** An add-on already on the line renders filled sky-blue with its count (`+30 Espresso Shot ×5`), the same at-a-glance state the Customize presets use. `addonLineId(addonId, parentLineId)` moves to `utils.ts` so the modal can match an option to its attached line by the id convention `addAddon` writes.
- **Both modes end in this item's own breakdown.** Add-ons & Customize: Subtotal (menu price × qty) / Add-ons / Item total. Discount: Subtotal / Add-ons / Discount / Total, where Discount is the sum of *this line's* rows and Total is what the item comes to after them.
- **The modal no longer shows order-wide money.** Its `subtotal` and `grandTotal` props are gone; every figure in it belongs to the open item, so the rows always add up. A whole-order discount is not folded into a line's Discount row, since it belongs to the order rather than to any one item — the order's Subtotal / Total stay on the panel behind and on the review screen.
